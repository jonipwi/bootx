...
The Go backend and terminal UI consume these contracts on a mature supported host environment. Version `0.3.0-dev` may process one operator-confirmed public, non-sensitive local document without treating it as synthetic and provides a separate [`assist.law-clarity.v1`](15-law-clarity-logic.md) screening contract. It still has no user authentication, network retrieval, external-source authentication, persistent memory, legal authority, or external action. The current fixture corpus is only a baseline and must be expanded before the exit gate. No integration into the opaque BootX operating-system artifacts is authorized.
