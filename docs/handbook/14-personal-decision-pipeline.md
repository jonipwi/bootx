...
The Go backend and terminal UI consume these contracts on a mature supported host environment. Version `0.4.0-dev` may process one operator-confirmed public, non-sensitive local document without treating it as synthetic and provides a separate [`assist.law-clarity.v1`](15-law-clarity-logic.md) screening contract. A third, CLI-only [`assist.ethical-review.v1`](16-ethical-publication-review.md) capability may send an explicitly confirmed public, non-sensitive draft to the OpenAI Responses API after local declared-evidence scoring. That request uses strict structured output, `store:false`, no tools, no conversation state, no application persistence, and no external actions; it cannot lower a local warning or make the human decision.

The prototype still has no user authentication, network retrieval, external-source authentication, persistent memory, legal authority, automatic publication, or external execution. The current fixture corpus is only a baseline and must be expanded before the exit gate. No integration into the opaque BootX operating-system artifacts is authorized.
